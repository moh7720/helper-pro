...
function serializeChat(chat) {
    return {
        ...
        chart: chat.chart?.mermaid ? {
            summary: chat.chart.summary,
            mermaid: chat.chart.mermaid,
            generatedAt: chat.chart.generatedAt
        } : null
    };
}
    ...
    try {
        ...
        if (start !== -1 && end !== -1 && end > start) {
            ...
            try {
                ...
                return JSON.parse(escapeRawNewlinesInJsonStrings(jsonSlice));
            }
        }
        throw error;
    }
...
app.post("/api/flowchart", isLoggedIn, async (req, res) => {
    try {
        ...
        res.json({
            ...flowchart,
            chatId: chat ? String(chat._id) : req.body.chatId
        });
    } catch (err) {
...
app.get("/api/charts/:chatId", isLoggedIn, async (req, res) => {
    try {
        const chat = await Chat.findOne({
        ...
        if (!chat.chart?.mermaid) {
            throw createHttpError("No chart has been generated for this conversation yet", 404);
        }
        ...
        console.error("Load chart error:", err.message);
        res.status(err.statusCode || 500).json({ error: err.message });
    }
});
